...
class StrucClass {
    ...
    // Unpack a series of n elements of size s from array a at offset p with fxn
	private _UnpackSeries(n, s, a, p):any[]
	{
	 ...
	 */
	// Pack a series of n elements of size s from array v at offset i to array a at offset p with fxn
	private _PackSeries  (n, s, a, p, v, i)
	{
	 ...
	 */
    private _DeArray(a: number[], p: number, l: number): number[][] {
      return [a.slice(p, p + l)];
    }
    ...
    private _EnChar(a: number[], p: number, v: string): void {
      a[p] = v.charCodeAt(0);
    }
  
    private _DeInt(a: number[], p: number): number {
      const lsb = this.bBE ? (this.el.len - 1) : 0;
      const nsb = this.bBE ? -1 : 1;
      const stop = lsb + nsb * this.el.len;
      let rv = 0;
      let i = lsb;
      let f = 1;
      for (; i != stop; rv += (a[p + i] * f), i += nsb, f *= 256);
    ...
    private _EnInt(a: number[], p: number, v: number): void {
      const lsb = this.bBE ? (this.el.len - 1) : 0;
      const nsb = this.bBE ? -1 : 1;
      const stop = lsb + nsb * this.el.len;
      let i = lsb;
      v = (v < this.el.min) ? this.el.min : (v > this.el.max) ? this.el.max : v;
      for (; i != stop; a[p + i] = v & 0xff, i += nsb, v >>= 8);
    }
  
    private _DeString(a: number[], p: number, l: number): string {
      const rv: string[] = [];
      for (let i = 0; i < l; rv[i] = String.fromCharCode(a[p + i]), i++);
      return rv.join('');
    }
  
    private _EnString(a: number[], p: number, l: number, v: string): void {
      for (let t, i = 0; i < l; a[p + i] = (t = v.charCodeAt(i)) ? t : 0, i++);
    }
  
    private _De754(a: number[], p: number): number {
    ...
    private _En754(a: number[], p: number, v: number): void {
    ...
    private _lenLut = { 'A': 1, 'x': 1, 'c': 1, 'b': 1, 'B': 1, 'h': 2, 'H': 2, 's': 1, 'f': 4, 'd': 8, 'i': 4, 'I': 4, 'l': 4, 'L': 4 };
    private _elLut = {
	 ...
	 */
    public Unpack(fmt: string, a: number[], p?: number): any[] | undefined {
      this.bBE = (fmt.charAt(0) != '<');
      p = p ? p : 0;
      const re = new RegExp(this._sPattern, 'g');
      let m;
      let n;
      let s;
      const rv: any[] = [];
	 ...
	 */
    public PackTo(fmt: string, a: number[], p: number, values: any[]): boolean {
      this.bBE = (fmt.charAt(0) != '<');
      const re = new RegExp(this._sPattern, 'g');
      let m;
      let n;
...
//      return this.PackTo(fmt, new Array(this.CalcLength(fmt)), 0, values) ? a : [];
	 ...
	 */
    public CalcLength(fmt: string): number {
      const re = new RegExp(this._sPattern, 'g');
      let m;
      let sum = 0;
      while (m = re.exec(fmt)) {
